...
export default function KontekstnayaReklamaProcessSection({ title, text, image, items }: Props) {
  return (
      ...
      <Container>
        <div className={`${grid.grid2} ${styles.layout}`}>
            ...
            <div className={styles.timeline}>
              {items.map(([itemTitle, itemText], index) => {
                const icons = [
                  ...
                  <svg key="6" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>,
                  <svg key="7" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg>,
                ];
                return (
                  <article key={itemTitle}>
                    <span className={styles.iconWrap}>{icons[index % icons.length]}</span>
                    <div>
                      <h3>{itemTitle}</h3>
                      <p>{itemText}</p>
                    </div>
          ...
          <div className={styles.visualCard}>
            <Image src={image} alt="Процесс настройки контекстной рекламы в Яндекс Директ — Make All Perfect" width={1200} height={1500} sizes="(max-width: 768px) 100vw, 48vw" />
          </div>
        </div>
      </Container>
    </section>
