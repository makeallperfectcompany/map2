...
export default function VkAdsAnalytics({ content }: { content: VkAdsContent["analytics"] }) {
  return (
    <section className={`${styles.darkSection} ${styles.brandAnalytics}`} aria-labelledby="vkAdsAnalyticsTitle">
      <Container>
        <div className={styles.analyticsTop}>
          <div className={styles.analyticsIntro}>
            <span className={styles.darkCode}>{content.label}</span>
        ...
        <p className={`${styles.performanceBar} ${styles.performanceGradient}`}>{content.accent}</p>
      </Container>
    </section>
  );
}